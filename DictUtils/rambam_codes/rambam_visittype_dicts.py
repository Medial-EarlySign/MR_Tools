...
rambam_visittype_file = 'rambom_visittype_codes.csv'
rambam_visittype_dict_file = RAMBAM_DIR + '\\dict.visittype'

rambam_med_index_start = 10000

if __name__ == '__main__':
    section_type_list = ['ev_outpatient_visits']
    attr_name_list = ['visittype']
    if os.path.isfile(rambam_visittype_file):
        diag_df = load_rambam_diag(rambam_visittype_file)
    else:
        diag_df = read_rambam_xmls(rambam_xml_dir, section_type_list, attr_name_list, rambam_visittype_file)
    ...
    rambam_def.to_csv(rambam_visittype_dict_file, sep='\t', index=False, header=False)
    first_line = 'SECTION' + '\t' + 'VISIT\n'
    add_fisrt_line(first_line, rambam_visittype_dict_file)
    print('DONE')
