...
def Test(df: pd.DataFrame, si, codedir: str, workdir: str):
    ...
    for col in cols:
        ...
        if null_cnt/len(df) > 0.001:
            print('Failed! There are %d(%2.3f%%) missing values for signal %s in col %s'%(null_cnt, 100*null_cnt/len(df), sig_name, col))
            return False
        
        if null_cnt > 0:
            print('There are %d(%2.3f%%) missing values for signal %s in col %s'%(null_cnt, 100*null_cnt/len(df), sig_name, col))
