...
def read_zeros_dict(fname,dict):
    dict.clear()
    with open(fname,'r') as fdict:
        for line in fdict:
            elems = line.split(' ')
            elem = str(elems[1])+str(elems[3])
            #print elem
            dict[elem] = 1
...
def prep_labsig(name,fname,fzeros,fout,col_id,col_sig,col_date,col_val,parse_date):
    print "preparing ",name,"from file",fname

    # first we read the zeros panel file, in order to get rid of those
    zdict = {}
    read_zeros_dict(fzeros,zdict)
    ...
    with open(fname,'rb') as csvfile:
        ...
        for row in reader:
            ...
            elm = str(tup[0])+str(date)
            if (float(tup[3]) != 0) or (not (elm in zdict)):
                if tup != prev_tup and int(row[col_id]) != 0:
                    mylist.append(tup)
                prev_tup = tup
                if i==max_lines:
...
prep_labsig("Lab2004",in_dir2+"LabsData_2004.txt",out_dir+"PanelsOfZeros",out_dir+"lab2004",0,1,2,3,1)
prep_labsig("Lab2008",in_dir2+"LabsData_2008.txt",out_dir+"PanelsOfZeros",out_dir+"lab2008",0,1,2,3,1)
prep_labsig("Lab2012",in_dir2+"LabsData_2012.txt",out_dir+"PanelsOfZeros",out_dir+"lab2012",0,1,2,3,1)
prep_labsig("Lab2015",in_dir2+"LabsData_2015.txt",out_dir+"PanelsOfZeros",out_dir+"lab2015",0,1,2,3,1)
...
#less Registry.21Dec15 | awk -F"\t" '(NR>1){a=$4; if(a=="")a=9;print $1"\t"a"\t"$2"\t"$3}' | sed 's/  */ /g' | sed 's/ ,/,/g' | sed 's/, /,/g' | sed 's/ \//\//g' | sed 's/\/ /\//g' | sed 's/ /_/g' > registry
#less Registry.21Dec15 | awk -F"\t" '(NR>1){print $3}' | sort | uniq | sed 's/  */ /g' | sed 's/ ,/,/g' | sed 's/, /,/g' | sed 's/ \//\//g' | sed 's/\/ /\//g' | sed 's/ /_/g' > reg_names
