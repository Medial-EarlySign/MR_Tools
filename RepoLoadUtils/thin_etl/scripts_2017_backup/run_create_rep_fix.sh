...
cp ${base_folder}/ID2NR ${base_folder}/repository_data_final
...
$MR_ROOT/Tools/Flow/Linux/Release/Flow --rep_create --convert_conf ${base_folder}/rep_configs/thin.fix_specific.convert_config 2>&1 | tee ${base_folder}/outputs/thin.fix_specific.convert_config.log

has_err=`grep -i "error"  ${base_folder}/outputs/thin.fix_specific.convert_config.log | wc -l`
if [ $has_err -eq 0 ]; then
        echo "repository created successfully :). creating op index"
