...
def getRandomClusters(data, factors, max_value):
    factorGrps = [] 
    meani = list()
    stdi = list()
    cnti = list()
    ...
    while do_again:
        ...
        for i in range(len(factors)):
            ...
            factorData_vals = list(filter( lambda x: x<=max_value ,factorData))
            mean_all, std_all = np.mean(factorData_vals), np.std(factorData_vals)
            meani.append(mean_all)  
            stdi.append(std_all)
            cnti.append(len(factorData))
    return [factorGrps, meani, stdi, cnti]

def fixFactorsMixtureGaussian(data, factors, fix_mean_threshold, max_value, commonVal = None, forceStd = False, out_file_fixed = None):
    max_iters = 1000
    if len(factors) <= 0:
        return data
    ...
    #init clusters:
    factors.append(1.0) 
    #factorGrps, meani, stdi, cnti = getRandomClusters(data, factors, max_value)
    ...
    while changes > 0 and iter < max_iters:
        ...
        for i in range(len(data)):
            ...
            for k in range(len(factors)):
                ...
                if stdC == 0:
                    stdC = 0.1
                    print('small std in factor=%2.3f cnt=%d, mean=%2.3f'%(factors[k], cnti[k], meani[k]))
                    if out_file_fixed is not None:
        ...
        for k in range(len(factors)):
            ...
            if len(factorInds) == 0:
                print('warnning empty group found factor=%2.3f! iter=%d, meani[k]=%2.3f, stdi[k]=%2.3f, cnti[k]=%d, allMeans=%s'%(factors[k], iter, meani[k], stdi[k], cnti[k], meani))
                if out_file_fixed is not None:
                ...
                continue
            th_per=0.01
            factorData = sorted(list(map( lambda i: data[i] ,factorInds)))
            min_th=factorData[int(len(factorData)*th_per)]
            cnti[k] = len(factorData)
            max_th=factorData[min(int(len(factorData)*(1-th_per)), len(factorData)-1)]
            if max_value< max_th:
                max_th=max_value
            factorData = list(filter( lambda x: x != 0 and x <= max_th and x >= min_th, factorData)) # zeros do not count when calculating mean and std. And exclude outliers
            previusStd = stdi[k]
            previousMean = meani[k]
            meani[k], stdi[k] = np.mean(factorData),  np.std(factorData)
            
            if cnti[k] < 50: # small sample
                print('warnning small cluster factor=%2.3f. iter=%d, meani[k]=%2.3f->%2.3f, stdi[k]=%2.3f->%2.3f, cnti[k]=%d. all_means=%s'%(factors[k],iter, previousMean, meani[k], previusStd, stdi[k], cnti[k], meani))
                if out_file_fixed is not None:
            ...
            if stdi[k] > 1000:
                print('warnning std is big in factor=%2.3f. iter=%d. stdi[k]=%2.3f, cnti[k]=%d. all_means=%s'%(factors[k],iter,  stdi[k], cnti[k], meani))
                if out_file_fixed is not None:
            ...
            if forceStd and ((meani[k]/(commonVal/factors[k])) >= 10 or (meani[k]/(commonVal/factors[k])) <= 0.1):
                print('mean is too far.. iter=%d, factor=%2.3f, mean=%2.3f'%(iter, factors[k], meani[k]))
                if out_file_fixed is not None:
    ...
    #print('f_matches= ', f_matches, ' meani=', meani, ' factors=', factors)
    for k in range(len(f_matches)):
        ...
        if finalMean == 0 or minDf / finalMean >= fix_mean_threshold:
            print('f_matches= ', f_matches, ' iters=', iter)
            of=open('/tmp/debug.txt', 'w')
            of.writelines(map(lambda x: '%f\n'%(x) ,data))
            of.close()
            err_msg='change in mean of more than %f%% - Error. has %2.3f%% change. debug data: /tmp/debug.txt'%(fix_mean_threshold*100 ,100*minDf / finalMean if finalMean >0 else 0)
            print(err_msg)
    ...
    if changes > 0:
        print('Didn''t converge - has %d changes in last round'%changes)
        if out_file_fixed is not None:
...
if __name__ == "__main__":
    ...
    testData = x1 + x2
    #fixed = fixFactorsMixtureGaussian(testData, factor, 0.1, 1e6, 14) 
    fixed = fixFactorsMixtureGaussian(testData, factor, 0.1, 1e6, None) # without common value if n2 is effecting common value, than we will have error
