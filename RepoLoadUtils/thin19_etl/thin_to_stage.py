...
from staging_config import ThinLabs, ThinDemographic, ThinReadcodes
...
columns_map_cat = {'combid': 'orig_pid', 'ahdcode': 'code', 'description': 'description', 'lookupdesc': 'value',
...
def cats_to_sql(db_engine, thin_def,  cat_df, ahd_lookup):
    ...
    cat_df['value'] = cat_df['value'].astype(str).str.replace(',', ' ')
    cat_df['unit'] = np.NaN
...
def clinic_to_sql(db_engine, thin_def,  clinc_df, ahd_codes, read_codes):
    ...
    #clinc_df = clinc_df[['combid', 'eventdate',0 , 'ahdcode', 'description', 'medcode', 'source',
    #                     'level_5', 'filed_desc']].merge(read_codes, how='left', on='medcode')
...
def load_demography():
    ...
    for demo_df in pd.read_csv(pat_demo_file, chunksize=500000):
