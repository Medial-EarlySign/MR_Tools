...
dfCaseDx =  pd.read_csv(sourceDir + fileCaseDx, delimiter = '\t',encoding = "ISO-8859-1",dtype = str) 
...
dfControlDxUnqiueICD9 = createUniqueIcd9Df(dfControlDx)
...
def cancersPerType(df,caseControlType):
    ...
    cancerNumPerType[0:30].plot(kind='bar')
...
plt.figure()
dfComb[0:30].plot(y = ['Case','Control'],kind = 'bar')
...
# Dx No Cancer
list(xxx)[1] # Nothing  no hosp
list(xxx)[23] # Nothing  no hosp
