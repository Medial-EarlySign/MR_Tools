...
def codes_to_cancer_location(stage_def):
    cfg = Configuration()
    ...
    read_codes['close_rc'] = read_codes['medcode'].apply(lambda x: get_close_rc(x, codes_2_details_df[['short_code', 'code']]))
    ...
    for i, x in merge_close[['medcode', 'desc', 'organ']].drop_duplicates().iterrows():
        print(x['medcode'][:7] + ', ' + x['desc'] + ', ' + x['organ'] )
    #close_codes = [get_unique_outfile(x) for x in codes]
    print(' Reading ' + str(len(rcs)) + ' Readcodes')
    cnt = 1
    for rccd in rcs:
    #for rccd in ['B339.00']:
        print(str(cnt) + ' reading readcode ' + rccd)
        cnt+=1
        sql_query = "SELECT orig_pid, code as medcode, date1 as eventdate FROM " + stage_def.TABLE + " WHERE code = '" + rccd + "'"
        print(sql_query)
        one_df = pd.read_sql_query(sql_query, db_engine)
        one_df['medialid'] = one_df['orig_pid'].astype(str).map(id2nr_map)
        signal_df = signal_df.append(one_df[['medialid', 'medcode', 'eventdate']], ignore_index=True)
    ...
    #print(pracs)
    for pr in pracs:
        ids = id2nr_map.loc[id2nr_map.index.str.startswith(pr)].values.tolist()
        signal_df_pr = signal_df[signal_df['medialid'] .isin(ids)]
        print(' Processing parc ' + pr)
        reg_df = signal_df_pr[['medialid', 'eventdate', 'medcode']].merge(merge_close, how='left',on='medcode')
