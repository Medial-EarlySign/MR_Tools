...
MR_ROOT = '/nas1/UsersData/avi/MR/'
#MR_ROOT = os.environ['MR_ROOT']
...
def load_hyper_reg():
    reg_creator_app = fixOS(os.path.join(MR_ROOT, 'Tools', 'Registries', 'CreateRegistries', 'Linux', 'Release', 'Hypertension'))
    cfg = Configuration()
...
def load_cvd_reg(DiseaseReadCodesFileN,PastReadCodesFileN,Runtype,SignalName ):
    reg_creator_app = fixOS(os.path.join(MR_ROOT, 'Tools', 'Registries', 'CreateRegistries', 'Linux', 'Release', 'CVD'))
    cfg = Configuration()
    work_path = fixOS(cfg.work_path)
    reg_path = os.path.join(work_path, 'registry')
    rep_path = fixOS(cfg.repo)
    ...
    list_folder = os.path.join(MR_ROOT, 'Tools', 'Registries', 'Lists')
...
def load_diabetes_reg():
    reg_creator_app = fixOS(os.path.join(MR_ROOT, 'Tools', 'Registries', 'createDiabetesRegistry', 'Linux', 'Release', 'Diabetes'))
