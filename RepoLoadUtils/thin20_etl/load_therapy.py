...
from const_and_utils import read_fwf, eprint
from utils import to_float, is_nan, read_tsv, fixOS, write_tsv
...
def load_therapy():
    ...
    id2nr = get_id2nr_map()
    in_file_names = list(filter(lambda f: f.startswith('therapy.j'), os.listdir(data_path)))
...
def sort_files():
    ...
    files = ['h_therapy', 'i_therapy', 'j_therapy'] #,'d_therapy', 'e_therapy']
    for file in files:
