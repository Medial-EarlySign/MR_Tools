...
def prep_smoking_intensity(rep, cfg):
    ...
    smoking_intnsity = smoking_quantity[~(smoking_quantity.val2 == -9)]
    smoking_intnsity.loc[:, 'sig_name'] = sig
    smoking_intnsity.sort_values(by=['pid', 'date'], inplace=True)
    if os.path.exists(os.path.join(out_folder, sig)):
        os.remove(os.path.join(out_folder, sig))
    write_tsv(smoking_intnsity[['pid', 'sig_name', 'date', 'val2']], out_folder, sig)
...
def prep_smoking_status(rep, cfg):
    ...
    rep_dir = fixOS(cfg.repo)
    print(rep_dir)
    rep.read_all(rep_dir, [], ['RC', 'Smoking_quantity', 'GENDER'])
    ...
    print('3')
    cols = ['pid', 'date', 'smoke_stat']
    for i in range(0, len(pids), chunk_size):
        curr_list = pids[i:i+chunk_size]
        rc_df = rep.get_sig('RC', pids=curr_list, translate=False)
        print(curr_list[0])
        rc_df_smk = rc_df.merge(dict_df, how='left', left_on='val', right_on='codeid')
        ...
        un_certen = pd.merge(un_certen, sig_qaun, how='left', on=['pid', 'date'])
        un_certen = un_certen[un_certen['val2'] > 0]
    ...
    cols = ['pid', 'signal', 'date', 'smoke_stat']
    rc_df_smk_all.sort_values(by=['pid', 'date'], inplace=True)
    if os.path.exists(os.path.join(out_folder, sig)):
        os.remove(os.path.join(out_folder, sig))
