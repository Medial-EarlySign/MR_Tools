...
def initClusters(data, factors, commonVal=None, out_file_fixed=None):
    ...
    if commonVal is None or (type(commonVal) == str and len(commonVal) == 0) or np.isnan(commonVal):
        # TODO: handle 0 as most common value
        #valHist = hist(data)
        val_hist = dict(data.value_counts())
        tups = sorted(val_hist.items(), key=lambda kv: kv[1], reverse=True)
...
def fixFactorsMixtureGaussian(data, factors, commonVal=None, forceStd=True, out_file_fixed=None):
    cfg = Configuration()
    max_iters = 1000
    if len(factors) <= 0:
        return data
    ...
    data_df['bestScore'] = np.nan
    data_df['bestFactor'] = 0
    while changes > 0 and iter < max_iters:
        ...
        for k in range(len(factors)):
            ...
            if stdC == 0:
                ...
                if out_file_fixed is not None:
                    out_file_fixed.write(
                        'small std in factor=%2.3f cnt=%d, mean=%2.3f' % (factors[k], cnti[k], meani[k]))
            if forceStd:
                stdC = 1.0 / math.sqrt(factors[k])
            data_df['z'] = (data_df['val'] - meani[k]) / stdC
            data_df['currScore'] = data_df['z'] * data_df['z']
            change_cnt = data_df[(data_df['bestScore'].isnull()) | (data_df['currScore'] < data_df['bestScore'])].shape[0]
            data_df['bestFactor'] = data_df['bestFactor'].where((data_df['bestScore'].notnull()) & (data_df['currScore'] >= data_df['bestScore']),  k)
            data_df['bestScore'] = data_df['bestScore'].where((data_df['bestScore'].notnull()) & (data_df['currScore'] >= data_df['bestScore']),  data_df['currScore'])
        changes += change_cnt > 0
        factorGrps = data_df['bestFactor'].values
        ...
        try:
            for k in range(len(factors)):
                factorInds = [x for x in range(len(factorGrps)) if factorGrps[x] == k]
                if len(factorInds) == 0:
                    eprint(
                        'warnning empty group found factor=%2.3f! iter=%d, meani[k]=%2.3f, stdi[k]=%2.3f, cnti[k]=%d, allMeans=%s' % (
                        factors[k], iter, meani[k], stdi[k], cnti[k], meani))
                    if out_file_fixed is not None:
                        out_file_fixed.write(
                            'warnning empty group found factor=%2.3f!! iter=%d, meani[k]=%2.3f, stdi[k]=%2.3f, cnti[k]=%d, allMeans=%s' % (
                            factors[k], iter, meani[k], stdi[k], cnti[k], meani))
                ...
                factorData = data[factorInds]
                factorData = factorData[(factorData < (10 * factorData.median())) & (factorData > (0.1 * factorData.median()))]
                factorData = list(factorData[factorData != 0])
                ...
                if cnti[k] < 50:  # small sample
                    eprint(
                        'warnning small cluster factor=%2.3f. iter=%d, meani[k]=%2.3f->%2.3f, stdi[k]=%2.3f->%2.3f, cnti[k]=%d. all_means=%s' % (
                        factors[k], iter, previousMean, meani[k], previusStd, stdi[k], cnti[k], meani))
                    if out_file_fixed is not None:
                        out_file_fixed.write(
                            'warnning small cluster factor=%2.3f. iter=%d, meani[k]=%2.3f->%2.3f, stdi[k]=%2.3f->%2.3f, cnti[k]=%d. all_means=%s' % (
                            factors[k], iter, previousMean, meani[k], previusStd, stdi[k], cnti[k], meani))
                ...
                if stdi[k] > 1000:
                    eprint('warnning std is big in factor=%2.3f. iter=%d. stdi[k]=%2.3f, cnti[k]=%d. all_means=%s' % (
                    factors[k], iter, stdi[k], cnti[k], meani))
                    if out_file_fixed is not None:
                        out_file_fixed.write(
                            'warnning std is big in factor=%2.3f. iter=%d. stdi[k]=%2.3f, cnti[k]=%d. all_means=%s' % (
                            factors[k], iter, stdi[k], cnti[k], meani))
                    stdi[k] = 1000
                if forceStd and (
                        (meani[k] / (commonVal / factors[k])) >= 10 or (meani[k] / (commonVal / factors[k])) <= 0.1):
                    eprint('mean is too far.. iter=%d, factor=%2.3f, mean=%2.3f' % (iter, factors[k], meani[k]))
                    if out_file_fixed is not None:
                        out_file_fixed.write(
                            'mean is too far.. iter=%d, factor=%2.3f, mean=%2.3f' % (iter, factors[k], meani[k]))
                    meani[k] = commonVal / factors[k]
            iter += 1
        except:
            eprint('Error In iter %d' % iter)
            traceback.print_exc()
            raise
    ...
    print('reference guassians mean is %2.3f with factor %2.3f - after multiply=  %2.3F' % (
    meani[minimal_mean_ind], f_matches[minimal_mean_ind], finalMean))
    if out_file_fixed is not None:
        out_file_fixed.write('reference guassians mean is %2.3f with factor %2.3f - after multiply=  %2.3F' % (
        meani[minimal_mean_ind], f_matches[minimal_mean_ind], maxFactor * minimalMean))
    ...
    for k in range(len(f_matches)):
        ...
        f_matches[k] = factors[
            bestInd]  # the best matching factor for this k guassian is ind. dont allow bestInd again?
        print(
            'validting matching guassians to factors. mean=%2.3f factor=%2.3f after_multiply=%2.3f with diff=%2.3f' % (
        ...
        if finalMean == 0 or minDf / finalMean >= cfg.fix_mean_threshold:
            ...
            raise NameError('change in mean of more than %f%% - Error. has %2.3f%% change' % (
            cfg.fix_mean_threshold * 100, 100 * minDf / finalMean if finalMean > 0 else 0))

    # set data:
    fac_map = pd.Series({i: f_matches[i] for i in range(len(f_matches))})
    fac_s = pd.Series(factorGrps)
    data = data * fac_s.map(fac_map)
    ...
    if changes > 0:
        ...
        if out_file_fixed is not None:
            out_file_fixed.write(
                "Didn''t converge - has %d changes in last round. factor group sizes: [%s]\n" % (changes, grpSizesStr))
    else:
        print('fix factors Converged after %d iters!!' % iter)
        if out_file_fixed is not None:
            out_file_fixed.write(
                "fix factors Converged after %d iters!! factor groups sizes: [%s]\n" % (iter, grpSizesStr))
    return data
...
def fix_distribution():
    ...
    read_folder = os.path.join(out_folder, 'FinalSignals2')
    write_folder = os.path.join(out_folder, 'FinalSignals_fixed')
    existing_files = os.listdir(write_folder)
    factor_df = read_tsv('Factors.txt', code_folder, names=['Signal', 'Factor', 'SupposeMean'])
    for _, rw in factor_df.iterrows():
        sig_name = rw['Signal']
        if sig_name in existing_files:  # or sig == 'BP':
            print('signal ' + sig_name + ' already fixed')
            continue
        print('===== ' + sig_name + ' ======')
        factor = rw['Factor']
        suppose_mean = rw['SupposeMean']
        data = pd.read_csv(os.path.join(read_folder, sig_name), sep='\t', names=['pid', 'sig','eventdate', 'val'],
                           usecols=[0, 1, 2, 3], dtype={'pid': int, 'eventdate': int, 'val': np.float64})
        data['val'] = fixFactorsMixtureGaussian(data['val'], [factor], suppose_mean)
        print(data)
        write_tsv(data, write_folder, sig_name)
