...
def get_demographic_files(cfg,id2nr):
    inFile = os.path.join(cfg.data_path,'AS_data_sharing.xlsx')
    pat = pd.read_excel(inFile)
    print(f'Read {len(pat)} demographics entries')
    pat['pid'] = pat.id.map(id2nr)
    ...
    pat[['pid','signal','Gender']].drop_duplicates(subset='pid').to_csv(outFile, sep='\t', index=False, header=False)

    # BYEAR + BDATE
    print('Doing BYEAR/BDATE')
    pat['signal'] = 'BYEAR'
    pat['date_of_birth']=pat.procedure_date-pd.to_timedelta(365.25*pat.age_at_echo,unit='D')
    pat['BYEAR'] = pd.DatetimeIndex(pat.date_of_birth).year
    ...
    print(pat)
    pat[['pid', 'signal', 'BYEAR']].drop_duplicates(subset='pid').to_csv(outFile, sep='\t', index=False, header=False)

    pat['signal'] = 'BDATE'
    pat['date_of_birth'] = 0
    outFile = os.path.join(path,'BDATE')
    pat[ ['pid', 'signal', 'date_of_birth']].drop_duplicates(subset='pid').to_csv(outFile, sep='\t', index=False, header=False)
