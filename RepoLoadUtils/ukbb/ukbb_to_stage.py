...
def load_ahd():
    ...
    for in_file_name in in_file_names:
        ...
        ahd_df['eventdate'] = pd.to_datetime(ahd_df['eventdate'], format='%Y%m%d', errors='coerce')
...
def load_medical():
    ...
    to_proc = 'medical.a6796'
    #start_proc = 'medical.f6749'
    for in_file_name in in_file_names:
        cnt = cnt + 1
        pracid = in_file_name[-5:]
        if in_file_name > to_proc:
        ...
        medical_df['eventdate'] = pd.to_datetime(medical_df['eventdate'], format='%Y%m%d', errors='coerce')
        ...
        medical_df['enddate'] = pd.to_datetime(medical_df['enddate'], format='%Y%m%d', errors='coerce')
