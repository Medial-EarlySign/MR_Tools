...
if __name__ == "__main__":
    ...
    to_table = 'eligibility_criterias_nlp'
    ...
    one_id = ""
    #one_id = " WHERE id_info_nct_id = 'NCT02280395'"
    query = query + one_id
    ...
    for ct_df in pd.read_sql_query(query, db_url, chunksize=100000):
        print(cnt)
        cnt += 1
        criterias_list = []
        for i, r in ct_df.iterrows():
            ...
            if not r['criteria']:
                continue
            criteria = r['criteria'].replace('\n\n', ' .\n')  #.replace(';', '.\n').replace(',', '.\n')
            criterias = nltk.sent_tokenize(criteria)
        ...
        file_name = to_table+str(cnt)+'.txt'
        full_file = fixOS('/nas1/Work/Users/Tamar/ClinicalTrials_load/'+file_name)
        eligibility_df[['nct_id', 'criteria', 'type']].to_csv(full_file, sep='\t', index=False)
        print('    saved ' + str(eligibility_df.shape[0]) + ' criteria to file ' + file_name + ' in ' + str(time.time() - time1))
