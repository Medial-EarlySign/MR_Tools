...
def GetBpTuple(group):
    """
    TrinetX dataset may contain mulitple measurements of Systolic (LOINC,8480-6) and Diastolic (LOINC,8462-4)
    blood pressure. These measurements are not necessarily aligned.
    Our options to handle these multiple unaligned measurements are as follows:

    * select first measurement of Systolic/Diastolic on each day
    * select last measurement  of Systolic/Diastolic on each day
    * select mean/median measurement  of Systolic/Diastolic on each day

    In this function we implement the first of these options

    """

    gsys = group[group['code'] == '8480-6']
    gdia = group[group['code'] == '8462-4']
    if len(gsys) > 0 and len(gdia) > 0:
        return (gsys['value'].values[0], gdia['value'].values[0])
    else:
        return np.nan

def ua_fix_units(lab_result_num_val):
    if lab_result_num_val >= 0.5 or lab_result_num_val == 0.2 or lab_result_num_val == 0.3 or lab_result_num_val == 0.4:
        lab_result_num_val = lab_result_num_val/1000
    return lab_result_num_val

def fix_uc2(x):
    """

    Conversion factor was caluclated based on a shift between two modes of
    the bimodal log-historgram. We need to check which units were used in the left
    (incorrect) mode and understand which conversion factor to use
    [in case of UC2 it is quite likely that factor equals 100]

    :param x:
    :return:
    """
    thrs = (np.exp(-0.7)-0.0001)/0.0884
    if x < thrs:
        x*=94.412
    return x
...
def get_lab_result_files(cfg, id2nr):
    ...
    *  Read conversion rules from lab_tests.csv
    *  Read target signal resolutions from lab_res.csv
    *  For each signal from the conversion list
        export data into a separate file within FinalSignals/ directory
    *   Load the signal data from 'FinalSignals/' directory
    *   Drop all rows that contain Null value
    *   Round values according to resolution information
    *   Sort by pid,date
    *   Overwrite signal file in the 'FinalSignals/' directory

    TODO:
        *   Implement conversion of uints
        *   Implement warning about missing signals
        *   Implement loading "vital signs" signals
    ...
    # Read conversion rules from .csv with columns:
    # Signal,code_description,code_system,code,unit_from,unit_to,unit_factor
    testsFile = os.path.join(cfg.code_path, 'lab_tests.csv')
    tests = pd.read_csv(testsFile)

    # Read resolutions
    # [IS] Q: What  happens when resolution is not present for some signal ?
    ...
    use_existing = False
    if not use_existing:
        # Read Admissions for filtering out
        ...
        # Read actual lab results from RAW TrinetX data
        inFile = os.path.join(cfg.data_path, 'lab_result.csv')
        # Process data in chunks
        for idx, lab in enumerate(pd.read_csv(inFile, chunksize=chunksize)):

            # in debug mode exit after processing the first chunk
            if bDEBUG and idx > 0:
                print(f'DEBUG: Exit after processing one chunk')
                break
            ...
            # Loop on signals to be loaded
            for signal in tests.Signal.unique():

                # Get all conversion rules that correspond to current signal
                testDf = tests.loc[tests.Signal == signal]
                testLabs = pd.DataFrame()

                # Loop over conversion rules
                for index, row in testDf.iterrows():
                    ...
                    # append to "testLabs" all test results that correspond to specific code
                    # e.g. code 61152-5 of LOINC

                    # get conversion factor for (code_system,code)
                    factor_df = testDf.loc[(testDf.code_system == code_system) & (testDf.code == code)]
                    if factor_df.shape[0] != 1:
                        print(f'code_system {code_system:}')
                        print(f'code        {code}')
                        print('factor_df:')
                        print(factor_df)
                        raise (ValueError(f'{factor_df.shape[0]} conversion factors found for {code}, expected 1'))

                    unit_factor = factor_df.iloc[0]['unit_factor']

                    # get data for (code_system,code)
                    lab_tmp = lab.loc[(lab.code_system == code_system) & (lab.code == code)].copy()

                    # Fix problematic signals
                    #========================

                    # UrineAlbumin [LOINC 1754-1]
                    if code_system == 'LOINC' and code == '1754-1':
                        lab_tmp['lab_result_num_val'] = lab_tmp['lab_result_num_val'].apply(ua_fix_units)

                     # UrineCreatinine [LOINC 14683-7]
                    if code_system == 'LOINC' and code == '14683-7':
                        lab_tmp['lab_result_num_val'] = lab_tmp['lab_result_num_val'].apply(fix_uc2)

                    # perform unit conversion
                    lab_tmp['lab_result_num_val'] *= unit_factor
                ...
                # append column which contains the name of current signal
                ...
                # Export collected signals to a file inside FinalSignals/ directory
                outFile = os.path.join(path, signal)
                testLabs[['pid', 'signal', 'date', 'lab_result_num_val', 'code']].drop_duplicates().to_csv(outFile,
                                                                                                           sep='\t',
                                                                                                           index=False,
                                                                                                           header=False,
                                                                                                           mode=mode)
...
def get_vital_result_files(cfg, id2nr):
    """
    ...
    *   Overwrite signal file in the 'FinalSignals/' directory
    ...
    # Read conversion rules from .csv with columns:
    # Signal,code_description,code_system,code,unit_from,unit_to,unit_factor
    testsFile = os.path.join(cfg.code_path, 'vitals_tests.csv')
    tests = pd.read_csv(testsFile)

    # Read resolutions
    # [IS] Q: What  happens when resolution is not present for some signal ?
    resFile = os.path.join(cfg.code_path, 'vital_res.csv')
    res = pd.read_csv(resFile)
    resDict = dict(zip(res.signal, res.res))
    ...
    if not use_existing:
        ...
        # Read actual lab results from RAW TrinetX data
        inFile = os.path.join(cfg.data_path, 'vitals_signs.csv')
        # Process data in chunks
        for idx, lab in enumerate(pd.read_csv(inFile, chunksize=chunksize)):
            ...
            for signal in tests.Signal.unique():
                ...
                # Export collected signals to a file inside FinalSignals/ directory
                outFile = os.path.join(path, signal)
                testLabs[['pid', 'signal', 'date', 'value', 'code']].drop_duplicates().to_csv(outFile,
                                                                                              sep='\t',
                                                                                              index=False,
                                                                                              header=False,
                                                                                              mode=mode)
    ...
    for signal in tests.Signal.unique():
        ...
        if signal == 'BP':
            print(f'Processing special (dual-channel) BP signal')
            # get BP tuples
            labs = labs.groupby(['pid', 'signal', 'date']).apply(GetBpTuple)
            # convert to dataframe
            labs = labs.reset_index(name='value')
            # drop rows where we couln'd find Systolic/Diastolic pair
            labs.dropna(inplace=True)

            labs['BPDia'] = labs['value'].apply(lambda x: x[1])
            labs['BPSys'] = labs['value'].apply(lambda x: x[0])

            labs.drop(columns='value', inplace=True)

            # sort by pid,date  [not really necessary here]
            labs = labs.sort_values(by=['pid', 'date'])
...
def get_admissions_file(cfg,id2nr):
    ...
    print('[IS] WARNING: Using executable from coby-internal to save time')
    exe = f'/nas1/UsersData/coby-internal/MR/Tools/RepoLoadUtils/trinetx_etl/unite_admissions/Linux/Release/unite_admissions'
...
if __name__ == '__main__':
    ...
    if bDEBUG:
        chunksize = 10000
        large_chunksize = 20000

    print('Loading configuration ...')
    cfg = Configuration()

    # Read ID2NR (mapping from TrinetX hash ids to integers)
    print('Reading ID2NR ...')
    id2nr = get_id2nr(cfg)
    ...
    # get_vital_result_files(cfg, id2nr)
    #
    # # Diagnosis (will _probably_ be required for Diabetes registry)
    # print('Loading diagnosis files ...')
