...
REP_NAME = 'jilin'
OUTDIR = '/server/Work/CancerData/Repositories/Jilin/jilin_jun19'
INDIR = '/server/Work/Users/Tamar/jilin_load/rep_configs'
...
def add_const_lines():
    cons_list = ['#', '# convert config file', '#', 'DESCRIPTION\t'+REP_NAME.upper()+' data - full version',
                 'RELATIVE \t1', 'SAFE_MODE\t1', 'MODE\t3', 'PREFIX\t'+REP_NAME+'_rep', 'CONFIG\t'+REP_NAME+'.repository',
                 'SIGNAL\t'+REP_NAME+'.signals', 'FORCE_SIGNAL\tGENDER, BYEAR',
                 'DIR\t'+INDIR+'',
                 'OUTDIR\t'+OUTDIR
                 ]
    return cons_list
...
if __name__ == '__main__':
    ...
    out_file = os.path.join(config_folder, REP_NAME+'.convert_config')
