...
# Write Procedures
def writeProcedures(dir,inId,outId,dictionary,outFile,logFile):
	...
	# Treatment
	lines,header = readers.read_lines(dir,"treatment",inId)
	for line in lines:
		id = int(line[header["patientunitstayid"]])
		if (id != inId):
			raise ValueError("Id Mismatch: Required %d Got %d"%(inId,id))	
		time = unitAdmitTime + timedelta(minutes=int(line[header["treatmentoffset"]]))
		treatment = line[header["treatmentstring"]]
		outFile.write("%s\tPROCEDURE\t%s\t%s\t%s\tcurrent\n"%(outId,time,time,treatment))
		dictionary[treatment] = []
		
	# RespiratoryCare
	lines,header = readers.read_lines(dir,"respiratoryCare",inId)
	for line in lines:
		id = int(line[header["patientunitstayid"]])
		if (id != inId):
			raise ValueError("Id Mismatch: Required %d Got %d"%(inId,id))	
...
def generate_procedures_files(config,signalId):
	...
	for rec in ids:
		...
		# Patients
		writeProcedures(config["DataDir"],id,newId,dictionary,outFile,logFile)
...
# Generate files
generate_procedures_files(config,args.signalId)
