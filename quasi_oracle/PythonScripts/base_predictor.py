...
def nn(x, nFeatures, nHidden, nLayers, actFunc, keep_prob):
    inSize = nFeatures
    layer = x
    ws = []
    bs = []
    for i in range(nLayers):
        w = weight_variable([inSize,nHidden])
        b = bias_variable([nHidden])
        if (actFunc == 'relu'):
            hidden = tf.nn.relu(tf.matmul(layer,w) + b)
        else:
            hidden = tf.nn.sigmoid(tf.matmul(layer,w) + b)
        layer = tf.nn.dropout(hidden, keep_prob)
    ...
    y = tf.nn.xw_plus_b(layer,final_w, final_b, name = 'prediction')

    return y,tf.trainable_variables()
   
def loss_function(y,y_,w_,vars,beta):
    loss = tf.reduce_mean(tf.multiply(w_,tf.nn.sigmoid_cross_entropy_with_logits(labels=y_, logits=y)))
...
def learn_model(FLAGS):
    ...
    data = read_and_split_data_set(FLAGS.data,FLAGS.test_p,FLAGS.treatment)
    ...
    with tf.device(device):
        ...
        # NN
        y,vars= nn(x,data.train.nfeatures,FLAGS.nhidden,FLAGS.nlayers,FLAGS.function,keep_prob)

        # Loss function
        loss = loss_function(y,y_,w_,vars,reg_beta)
    ...
    with tf.Session(config = tf.ConfigProto(allow_soft_placement = True)) as sess:
        ...
        for i in range(FLAGS.max_num_batches):
            ...
            if i % FLAGS.checkpoint_num_batches == FLAGS.checkpoint_num_batches - 1:
                train_loss= sess.run(loss, feed_dict={x: data.train.features, y_: data.train.labels, w_ : data.train.weights, keep_prob:1.0,reg_beta:0.0})
                ...
                if (min_test_loss==-1 or test_loss < min_test_loss):
                    ...
                    nStuck += 1
                print('step %d, training loss = %g test loss = %g Time = %f nStuck = %d' % (i,train_loss,test_loss,time.time()-start_time,nStuck))
...
def apply_model(FLAGS):
    data = read_data_set(FLAGS.data,FLAGS.treatment)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--treatment', type=str, default='Treatment', help='name of treatment column in matrix')
    ...
    parser.add_argument('--nlayers',type=int,default=2,help='# of hidden layers')
    parser.add_argument('--function',type=str,default='relu',help='activation function (sig/relu)')
